...
/**
 * Класс CategoryManagementActivity представляет собой активность для управления категориями в приложении.
 *
 * Основное назначение:
 * - Отображение списка категорий с возможностью добавления, редактирования и удаления.
 * - Интеграция с Firebase Firestore для работы с данными категорий.
 *
 * Основные функции:
 * - Загрузка категорий из базы данных Firestore и отображение их в RecyclerView.
 * - Создание новой категории через диалоговое окно.
 * - Редактирование существующей категории с обновлением данных в Firestore.
 * - Удаление категории с подтверждением через диалоговое окно.
 *
 * Поля:
 * - backIcon: Иконка для возврата на предыдущий экран.
 * - addCategoryButton: Кнопка для добавления новой категории.
 * - categoriesRecyclerView: RecyclerView для отображения списка категорий.
 * - categoryAdapter: Адаптер для управления отображением категорий.
 * - categoryList: Список категорий, загруженных из базы данных.
 * - db: Экземпляр FirebaseFirestore для работы с базой данных.
 *
 * Особенности:
 * - Использует кастомные диалоговые окна для добавления, редактирования и подтверждения удаления категорий.
 * - Интеграция с интерфейсом Header для консистентной навигации.
 * - Обработка ошибок с отображением уведомлений через Toast.
 */
