...
/**
 * Класс ReviewAdapter представляет адаптер для отображения списка отзывов в RecyclerView.
 *
 * Основное назначение:
 * - Управление отображением отзывов, включая текст, рейтинг, дату и действия (редактирование/удаление).
 * - Поддержка разного поведения в пользовательском и административном режимах.
 *
 * Основные функции:
 * - Отображение имени пользователя, текста отзыва, рейтинга и даты.
 * - В режиме администратора отображается название продукта и доступна только опция удаления отзыва.
 * - В пользовательском режиме пользователи могут редактировать и удалять свои собственные отзывы.
 *
 * Поля:
 * - reviews: Список отзывов (объекты Review).
 * - context: Контекст активности или фрагмента, где используется адаптер.
 * - actionListener: Интерфейс для обработки действий редактирования или удаления отзывов.
 * - auth: Экземпляр FirebaseAuth для проверки текущего пользователя.
 * - isAdminMode: Флаг, определяющий, активен ли режим администратора.
 *
 * Интерфейс:
 * - OnReviewActionListener: Определяет методы для обработки действий редактирования (onEditReview) и удаления (onDeleteReview) отзывов.
 *
 * Методы:
 * - onCreateViewHolder: Создаёт ViewHolder из layout-файла item_review.
 * - onBindViewHolder: Привязывает данные из объекта Review к ViewHolder.
 * - getItemCount: Возвращает количество отзывов.
 *
 * Вложенный класс:
 * - ReviewViewHolder: Хранит ссылки на элементы интерфейса, связанные с отзывом.
 *   Поля:
 *   - productName: Название продукта (отображается в режиме администратора).
 *   - userName: Имя пользователя.
 *   - reviewText: Текст отзыва.
 *   - dateText: Дата отзыва.
 *   - ratingBar: Рейтинг отзыва.
 *   - reviewActions: Контейнер для иконок действий (редактирование/удаление).
 *   - editReviewIcon: Иконка редактирования отзыва.
 *   - deleteReviewIcon: Иконка удаления отзыва.
 *
 * Особенности:
 * - В режиме администратора отображается название продукта и доступна только опция удаления отзывов.
 * - В пользовательском режиме действия редактирования и удаления доступны только для отзывов текущего пользователя.
 * - Отображение отзывов с различными уровнями доступа (администратор/пользователь).
 */
