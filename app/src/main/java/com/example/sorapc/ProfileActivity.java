...
/**
 * Класс ProfileActivity представляет собой активность для отображения и управления профилем пользователя.
 *
 * Основное назначение:
 * - Предоставление интерфейса для просмотра личных данных пользователя, списка привязанных карт, истории заказов и избранных товаров.
 * - Управление основными функциями профиля, такими как редактирование данных, добавление/удаление карт, смена пароля, удаление аккаунта и выход из профиля.
 *
 * Основные функции:
 * - Загрузка и отображение данных пользователя из Firebase Firestore.
 * - Поддержка редактирования данных пользователя с возможностью сохранения изменений в Firebase Firestore.
 * - Управление привязанными картами (добавление, удаление) с проверкой ограничений и валидацией.
 * - Удаление аккаунта с обязательным подтверждением через код верификации, отправляемый на email.
 * - Переход к экранам "Избранное", "История заказов" и "Админ-панель".
 * - Реализация функции выхода из аккаунта.
 *
 * Поля:
 * - surnameTextView, nameTextView, middleNameTextView, emailTextView, passwordTextView, phoneTextView: Поля для отображения данных пользователя.
 * - changePasswordTextView: Поле для перехода на экран смены пароля.
 * - editDataButton, favouritesButton, ordersHistoryButton, adminPanelButton, addCardButton, deleteAccountButton, logOutButton: Кнопки для управления действиями профиля.
 * - cardsList: RecyclerView для отображения списка привязанных карт.
 * - noCardsText: Поле для отображения сообщения об отсутствии привязанных карт.
 * - cardAdapter: Адаптер для работы со списком карт.
 * - verificationCode: Код подтверждения для удаления аккаунта.
 * - MAX_CARDS: Константа, ограничивающая количество привязанных карт (по умолчанию — 3).
 * - auth: Экземпляр FirebaseAuth для авторизации пользователя.
 * - db: Экземпляр FirebaseFirestore для работы с базой данных.
 *
 * Методы:
 * - loadUserData: Загружает и отображает данные пользователя из Firebase Firestore.
 * - loadCards: Загружает список привязанных карт из Firestore.
 * - updateCardsVisibility: Управляет видимостью списка карт в зависимости от их наличия.
 * - showEditDialog: Открывает диалог для редактирования данных профиля.
 * - updateUserData: Обновляет данные пользователя в Firestore.
 * - showAddCardDialog: Открывает диалог для добавления новой карты с валидацией данных.
 * - showConfirmDeleteCardDialog: Открывает диалог подтверждения удаления карты.
 * - showDeleteAccountDialog: Открывает диалог подтверждения удаления аккаунта.
 * - sendVerificationCode: Отправляет код подтверждения на email пользователя.
 * - showVerificationDialog: Открывает диалог для ввода кода подтверждения.
 * - deleteAccount: Удаляет аккаунт пользователя из Firebase Authentication и его данные из Firestore.
 * - openFavourites, openOrdersHistory, openAdminPanel: Методы для перехода на соответствующие экраны.
 * - logOut: Выполняет выход из аккаунта и перенаправляет пользователя на экран авторизации.
 *
 * Особенности:
 * - Интеграция с Firebase Firestore для загрузки, обновления и удаления данных.
 * - Поддержка отправки email через SMTP-сервер для подтверждения удаления аккаунта.
 * - Реализация ограничений на количество привязанных карт.
 * - Удобный и адаптивный интерфейс с использованием диалогов для ключевых операций.
 * - Интеграция с RecyclerView для управления списком карт с функцией свайпа для удаления.
 */
